...
class AnthbotDevice {
    ...
    async start() {
        if (!this.isValidSnId(this.sn)) {
            ...
            // (Re)starting is the only place pollingStopped should be cleared;
            // syncDevice()/schedulePolling() intentionally leave it alone so an
            // in-flight stop() can never be undone by a concurrent operation.
            this.pollingStopped = false;
    ...
    async syncDevice() {
        ...
        if (this.inPoll) {
            ...
            if (this.pollingTimeout) {
                ...
                // Cancel the stale timer directly (not via clearPolling()) so we don't also
                // set pollingStopped - that flag must only reflect an explicit stop()/disable(),
                // not this internal cadence reset, otherwise a concurrent stop() could be undone.
    ...
    schedulePolling() {
        ...
        this.pollingTimeout = this.adapter.setTimeout(async () => {
            ...
            if (!this.pollingStopped && !this.pollingTimeout) {
                this.schedulePolling();
            }
        }, POLLING_INTERVAL_MS);
    }

...
const SCHEDULE_DAYS_OVERDUE_TO_FORCE_LONG_DURATION_CUSTOM_AREAS = 2;
...
class AnthbotDevice {
    ...
    constructor(options) {
        ...
        // True when adapter is running (ie. active)
        this.running = false;
    ...
    /**
     * Verify a given serial number or custom area ID is valid.
     *
     * It is better to detect an invalid value, warn and ignore it
     * than to try to 'sanitise' a string as multiple 'dirty' strings
     * may be 'sanitised' to the same output.
     *
     * @param {string} toCheck S/N or ID to check
     * @returns true if S/N or ID is valid (ie. is alphanumeric)
     */
    isValidSnId(toCheck) {
        return /^[a-z0-9]*$/i.test(toCheck);
    }
     ...
     */
    async start() {
        if (!this.isValidSnId(this.sn)) {
            // Device SN isn't alphanumeric which could mess up object creation so ignore this
            this.log.warn(`Device serial looks invalid - ignoring (${this.sn})`);
        } else if ((await this.getStateAsync('command.disable'))?.val) {
            this.log.info(`Device is disabled (${this.sn})`);
        } else {
            this.log.info(`Starting (${this.sn})`);
            this.running = true;
     ...
     */
    stop() {
        this.log.info(`Stopping (${this.sn})`);
        this.running = false;
    ...
    async disable(disable) {
        await this.setStateChanged('command.disable', disable);
        if (disable) {
            this.stop();
        } else {
            await this.start();
    ...
    async createObjects() {
        ...
        const readOnlyStates = [
            // Shadow properties
            ['status.4g_state', 'boolean', 'sensor', '4G state'],
            ...
            ['status.wifi_state', 'boolean', 'sensor', 'WiFi state'],
            ...
            // Polling status
            ['status.last_poll', 'number', 'value.time', 'Last time Anthbot cloud finished a poll for this device'],
        ];
    ...
    async populateDeviceFromStates(customAreasIn) {
        ...
        for (const customAreaIn of customAreasIn) {
            if (!this.isValidSnId(customAreaIn.id)) {
                // Ignore custom area IDs that are not alphanumeric and could therefore break object
                // naming rules... which should never happen
                this.log.warn(`Custom area ID looks invalid - ignoring (${customAreaIn.id})`);
            } else {
    ...
    async syncDevice() {
        ...
        if (this.inPoll) {
            ...
            // Always cancel any timer running as it will be reset below if necessary
            this.clearPollingTimeout();

            // Set polling flag so any syncDevice calls are processed after
            this.inPoll = true;

            // Execute polling in do...while so a syncReq triggered in there (or by external command) causes the loop to repeat
            do {
                // We're doing a sync, so reset required flag
                this.syncReq = false;
                await this.pollDevice();
            } while (this.running && this.syncReq);

            // Done with the polling loop
            this.inPoll = false;

            if (!this.running) {
                // Device has been stopped so do nothing
                this.log.debug('Device stopped - no timer set after poll');
            } else {
                this.pollingTimeout = this.adapter.setTimeout(() => {
                    this.pollingTimeout = null;
                    this.syncDevice();
                }, this.adapter.config.pollingInterval * 1000);
            }
        } else {
            // checkClient must have returned false which is fatal
            this.log.error('No client available in sync, initiating reconnect');
    ...
    async pollDevice() {
        ...
        let success = await this.doDeviceCommand('get_all_props', 1, true);
        if (success) {
            // Wait a moment for their backend
            await new Promise(resolve => this.adapter.setTimeout(resolve, this.adapter.config.cloudSyncDelay, null));
        ...
        if (success) {
            // Update last_poll
            await this.setStateChanged('status.last_poll', Date.now());
    ...
    /**
     * Clear any timeout for the next sync of this device
     */
    clearPollingTimeout() {
    ...
    async setShadowState() {
        ...
        await this.setStateChanged(`status.mode`, this.mode);

        await this.setStateChanged(`status.active_area`, JSON.stringify(this.shadowState.active_area.id));

        await this.setStateChanged(`status.elec`, this.shadowState.elec.value);
        ...
        await this.setStateChanged(`status.mowing_area`, this.shadowState.mowing_area.value);
        await this.setStateChanged(`status.mowing_time`, this.shadowState.mowing_time.value);

        await this.setStateChanged(`status.rtk_state`, this.shadowState.rtk?.state == 1);
        await this.setStateChanged(`status.rtk_moved`, this.shadowState.rtk?.moved == 1);

        // map.area_id & map.plan_id are set only after map is load
        await this.setStateChanged(`map.map_area`, this.shadowState.map?.map_area?.value);

        // In command folder as are read/write

        // Visual Inspection
        await this.setStateChanged(`command.pobctl_level`, this.shadowState.device_config?.pobctl_level);
        await this.setStateChanged(`command.pobctl_switch`, this.shadowState.device_config?.pobctl_switch);

        // Rain perception
        await this.setStateChanged(`command.rain_switch`, this.shadowState.device_config?.rain_switch == 1);
        await this.setStateChanged(`command.rain_continue_time`, this.shadowState.device_config?.rain_continue_time);

        await this.setStateChanged(`command.volume`, this.shadowState.device_config?.volume);

        // Network state
        await this.setStateChanged(`status.4g_state`, this.shadowState.net_state?.['4g_state'] == 1);
        await this.setStateChanged(`status.wifi_state`, this.shadowState.net_state?.wifi_state == 1);
    }
    ...
    async checkCodeList() {
        ...
        if (success) {
            ...
            await this.setStateChanged(`status.code_list`, JSON.stringify(this.codeList));

            if (!Array.isArray(this.codeList) || this.codeList.length < 1) {
                this.log.warn('Code list is not valid or empty array');
            } else {
                const lastCode = this.codeList[0];
                await this.setStateChanged(`status.last_code`, lastCode.code);
                await this.setStateChanged(`status.last_code_text`, lastCode.event_message);
                await this.setStateChanged(`status.last_code_type`, lastCode.code_type);
            }
        }
    }
    ...
    async checkMapUpdates() {
        if (
            ...
            if (!deviceMapFiles) {
                this.log.warn('Empty device map returned');
            } else {
                this.processDeviceMapFiles(deviceMapFiles);
            }
        }
    }

    /**
     * Process device map files, creating channels for each custom area & saving schedule info
     *
     * @param {object} deviceMapFiles Object holding device map files
     */
    async processDeviceMapFiles(deviceMapFiles) {
        // Assume map load successful
        this.mapLoaded = true;
        ...
        if (!areaSetting) {
            this.log.warn('No area_setting.json found in device files');
            // Try again next poll
            this.mapLoaded = false;
        } else {
            ...
            await this.setStateChanged(`map.custom_areas.raw`, JSON.stringify(this.customAreasRaw));
            ...
            await this.setStateChanged(`map.ridable_areas.raw`, JSON.stringify(this.ridableAreas));
            ...
            for (const customArea of this.customAreas) {
                ...
                await this.setStateChanged(`map.custom_areas.${customArea.id}.mow_head`, customArea.mow_head);
                await this.setStateChanged(`map.custom_areas.${customArea.id}.cutter_height`, customArea.cutter_height);
            }
            ...
            await this.setStateChanged('map.area_id', areaSetting?.area_id);
        }
        ...
        if (!this.timeSetting) {
            this.log.warn('No time_setting.json found in device files');
            // Try again next poll
            this.mapLoaded = false;
        } else {
            await this.setStateChanged(`command.do_not_disturb`, this.timeSetting.no_disturb?.[0].active == 1);
            await this.setStateChanged(`command.do_not_disturb_start`, this.timeSetting.no_disturb?.[0].start_time);
            await this.setStateChanged(`command.do_not_disturb_end`, this.timeSetting.no_disturb?.[0].end_time);

            await this.setStateChanged('map.plan_id', this.timeSetting.plan_id);
        }
    }
    ...
    async checkCustomAreaTask() {
        ...
        if (
            ...
            if (!startCode) {
                ...
                if (Date.now() - lastStart < this.adapter.config.pollingInterval * 1000 * 2) {
            ...
            if (
                ...
                if (chargeIndex >= 0) {
                    ...
                    if (Date.now() - lastCharge < this.adapter.config.pollingInterval * 1000 * 2) {
                        this.log.warn(
            ...
            if (endIndex >= 0 && (endIndex < startIndex || startIndex === -1)) {
                ...
                if (this.shadowState.active_area.id.length == 1) {
                    ...
                    if (!singleArea) {
                        this.log.error(`Could not find custom area ID ${singleAreaId} when task finished`);
                    } else {
    ...
    async checkCustomAreaSchedule() {
        ...
        const todaysMowingWindowStart = suncalcTimes.civilDawn.ts + this.adapter.config.scheduleDawnOffset * 60 * 1000;
        if (now.getTime() < todaysMowingWindowStart || now.getTime() > suncalcTimes.civilDusk.ts) {
            // Before dawn or after dusk
            this.log.debug(`checkCustomAreaSchedule... time now is before dawn (+delay) or after dusk`);
            return;
        }
        const availableTimeToMow = suncalcTimes.civilDusk.ts - todaysMowingWindowStart;

        // Work out if we are 'early in the day'... ie. close to today's start time
        const isEarlyInTheDay =
            now.getTime() < todaysMowingWindowStart + this.adapter.config.pollingInterval * 1000 * 2;
        ...
        for (const customArea of customAreasLeftToday) {
            ...
            if (now.getTime() + customArea.estimatedElapsedTimeToUse > suncalcTimes.civilDusk.ts) {
                ...
                // If the last cut of a zone was interrupted then estimated time can be unreasonably large,
                // or maybe the zone is just very big. Either way, if there is no time left today, but the
                // zone is more than 2 days overdue, and it is early in the day, start it anyway.
                if (
                    isEarlyInTheDay &&
                    customArea.estimatedElapsedTimeToUse > availableTimeToMow &&
                    customArea.currentDaysOverdue > SCHEDULE_DAYS_OVERDUE_TO_FORCE_LONG_DURATION_CUSTOM_AREAS
                ) {
                    this.log.warn(
                        `Estimated time for ID ${customArea.id} is longer than day length, but ${customArea.currentDaysOverdue} days overdue so scheduling anyway`,
                    );
            ...
            if (startAreaId) {
                // Found something so start it...
                this.log.info(
                    `Scheduled start for ID ${startAreaId} (best of ${JSON.stringify(customAreaIdsLeftToday)})`,
                );
